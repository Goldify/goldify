 ...
 */
export const createGoldifyPlaylist = async (retrievedTokenData, userId, playlistName, playlistDescription) => {
  const headers = {
    headers: {
 ...
 * @param  {object} retrievedTokenData object containing tokens necessary for header
 * @param  {string} playlistName name of the Playlist to create
 * @return {object} The Goldify playlist, if it exists
 */
export const findExistingGoldifyPlaylistByName = async (retrievedTokenData, playlistName) => {
  const headers = {
    headers: {
      "Authorization": "Bearer " + retrievedTokenData.access_token
    },
